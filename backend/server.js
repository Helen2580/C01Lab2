...
// get all notes belong to the user
app.get("/getAllNotes", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // store all notes posted by the user
      let userNotes = [];
      await data.forEach(note => {
        userNotes.push(note);
      });
      ...
      if (userNotes.length == 0) {
        return res
          .status(404)
          .json({ error: "No notes are being posted by the user"});
      }
...
// Delete the node with a given ID for the user
  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (deleteData.deletedCount === 0) {
        return res
          .status(404)
          .json({ error: "Unable to delete note with given ID for the user." });
      }
...
// Edit note with the given ID for the user
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title || !content) {
      return res
        .status(400)
        .json({ error: "Title and content both needed to edit note." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (Data.matchedCount === 0) {
        return res
          .status(404)
          .json({ error: "Unable to find note with given ID for the user." });
      }
